...
impl std::io::Read for Reader {
    ...
    let nbytes = unsafe { htslib::bgzf_read(self.inner, buf.as_mut_ptr() as *mut libc::c_void, buf.len() as u64) };
		if nbytes < 0 {
			Err(std::io::Error::new(std::io::ErrorKind::Other, "Can not read"))
		} else {
			Ok(nbytes as usize)
		}
	}
...
mod tests {
    ...
    fn test_is_bgzip_plain() {
			assert!(!is_bgzip(FN_PLAIN).unwrap(), "Plain file not detected as BGZIP");
			assert!(!is_bgzip(FN_GZIP ).unwrap(), "Zip file not detected as BGZIP");
    ...
    fn test_open_plain() {
			...
			assert!(reading_result.is_ok(), "Reading plain file into buffer is ok");
			assert_eq!(reading_result.unwrap(), 190, "Reading plain file into buffer is correct size");
			assert_eq!(my_content, CONTENT, "Reading plain file with correct content");
    }
    ...
    fn test_open_gzip() {
			...
			let mut my_content = String::new();
			let reading_result = r_result.unwrap().read_to_string(&mut my_content);
			assert!(reading_result.is_ok(), "Reading gzip file into buffer is ok");
			assert_eq!(reading_result.unwrap(), 190, "Reading gzip file into buffer is correct size");
			assert_eq!(my_content, CONTENT, "Reading gzip file with correct content");
    }
    ...
    fn test_open_bgzip() {
			...
			let mut my_content = String::new();
			let reading_result = r_result.unwrap().read_to_string(&mut my_content);
			assert!(reading_result.is_ok(), "Reading bgzip file into buffer is ok");
			assert_eq!(reading_result.unwrap(), 190, "Reading bgzip file into buffer is correct size");
			assert_eq!(my_content, CONTENT, "Reading bgzip file with correct content");
    }
}
